...
class LoggerFactory:
    ...
    def get_logger(
        ...
        if LoggerFactory._root_logger is None:
            ...
            LoggerFactory._root_logger.setLevel(level)

            # 添加文件处理器
            file_handler = LoggerFactory._get_file_handler(level=logging.DEBUG)
            LoggerFactory._root_logger.addHandler(file_handler)

            # 添加控制台处理器
            console_handler = LoggerFactory._get_console_handler(level=logging.DEBUG)
